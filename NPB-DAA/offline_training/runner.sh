...
do
  ...
  echo "sh runner.sh -t ${train_data} -p ${phn_label} -w ${wrd_label} -l ${label} -b ${i} -e ${end}" >> continue.sh

  python train.py --train_data ${train_data} | tee log.txt
  echo "summary starting..." >> log.txt
  python summary.py --phn_label ${phn_label} --wrd_label ${wrd_label}
...
python summary_summary.py --result_dir ${label} >> log.txt
