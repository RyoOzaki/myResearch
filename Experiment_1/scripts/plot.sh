...
dir_names=`ls segmentation_tmp/${result_dir} | grep "^[0-9]*$" -o`

for dname in ${dir_names};
do
  echo "Processing ${result_dir}/${dname}"
  python src/Evaluate_summary/plot_segmentation_results.py \
    --model segmentation_tmp/${result_dir}/hypparams/model.config \
    --result_dir segmentation_tmp/${result_dir}/${dname}/results/ \
...
cp -r segmentation_tmp/${result_dir}/figures segmentation_result_figure/${result_dir}/
